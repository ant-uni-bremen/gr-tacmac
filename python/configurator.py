...
def generate_payload(
    timeslots=9, subcarriers=64, active_subcarriers=52, overlap=2, cp_len=16, ramp_len=8
):
    cs_len = ramp_len
    seed = utils.generate_seed("awesome frame")
    d = utils.get_random_qpsk(timeslots * active_subcarriers, seed)
    frame = modulate_mapped_gfdm_block(
        d, timeslots, subcarriers, active_subcarriers, overlap, 0.2, dc_free=True
    )
...
def generate_preambles(
    ...
    subcarrier_map = mapping.get_subcarrier_map(
        subcarriers, active_subcarriers, dc_free=True
    )
